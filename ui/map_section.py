...
def _bytes_export(cle_b2: str, titre: str, mode: str, fmt: str, fond: str, langue: str, opacite: float) -> bytes:
    data, bounds = charger_raster(cle_b2)
...
def render_map_section(
    ...
    if photo_url:
        ...
        with col_photo:
            ...
            st.markdown(
                f'<p style="font-size:0.75rem;color:#6b7280;margin-top:4px;">{legende_html}</p>',
                unsafe_allow_html=True,
            )
        with col_info:
            st.markdown(f"<h3 style='margin-bottom:8px;'><em>{espece}</em></h3>", unsafe_allow_html=True)
            st.markdown(
    ...
    # --- Remplace construire_chemin + racine par construire_cle ---
    try:
    ...
    # --- Remplace chemin_tif.exists() par une vérification B2 ---
    try:
        _get_s3().head_object(Bucket=B2_BUCKET, Key=cle_tif)
    except Exception:
        st.warning(t("map_fichier_introuvable", chemin=cle_tif))
        st.stop()
    ...
    if periode_cle == "current":
        ...
        titre_carte = t("map_titre_carte_futur", espece=espece, periode=periode_label, ssp=ssp_choisi)
    if est_binaire:
    ...
    OPTIONS_FOND = {"plan": t("map_fond_plan"), "satellite": t("map_fond_satellite")}
    ...
    carte = creer_carte_folium(
        data, bounds, mode=mode_figure, fond=fond_cle,
        opacite=opacite, langue=get_langue_courante(),
    )
    ...
    with dl1:
        with st.spinner(t("map_export_spinner")):
            png_data = _bytes_export(cle_tif, titre_carte, mode_figure, "png", fond_cle, langue, opacite)
        st.download_button(label="PNG", data=png_data, file_name=f"{nom_fichier}.png", mime="image/png", use_container_width=True)

    with dl2:
        with st.spinner(t("map_export_spinner")):
            jpg_data = _bytes_export(cle_tif, titre_carte, mode_figure, "jpeg", fond_cle, langue, opacite)
        st.download_button(label="JPG", data=jpg_data, file_name=f"{nom_fichier}.jpg", mime="image/jpeg", use_container_width=True)

    with dl3:
        with st.spinner(t("map_export_spinner")):
            pdf_data = _bytes_export(cle_tif, titre_carte, mode_figure, "pdf", fond_cle, langue, opacite)
        st.download_button(label="PDF", data=pdf_data, file_name=f"{nom_fichier}.pdf", mime="application/pdf", use_container_width=True)

    with dl4:
        # --- Remplace open(chemin_tif) par téléchargement depuis B2 ---
        buf = io.BytesIO()
        _get_s3().download_fileobj(B2_BUCKET, cle_tif, buf)
        st.download_button(label="TIF", data=buf.getvalue(), file_name=f"{nom_fichier}.tif", mime="image/tiff", use_container_width=True)
